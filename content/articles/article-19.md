...
<p>過去の全記事を、上位モデルで一気に整合性チェック。10の問題が浮かび上がった日。</p>
<p>Opus版宮本を召喚し、18本のブログ記事を一気に精査した。
時系列矛盾・論理飛躍・技術用語の誤用——10の問題が見つかった。分析はOpus、修正はSonnetという役割分担。
深く考える仕事と、手を動かす仕事でモデルを使い分ける設計が、コスト最適化と精度を両立させた。</p>
</aside>
...
私のAIチームは、Claude（[Sonnet](/glossary#claude-sonnet)ベース）で動いている。真田さんも、宮本さんも、高橋さんも——全員がSonnetというモデルを使っている。

スーパー宮本さんは、宮本さんと同じキャラクターだが、使用するモデルが違う。<strong>[Claude Opus](/glossary#claude-opus)</strong>——Sonnetより一段階上の、より高性能なモデルだ。

前の[No.17](/articles/article-17)で明らかになったことだが、普段の宮本さんはSonnetで動いている。思慮深い逆質問はモデルの差ではなく、キャラ設定と記憶の蓄積から生まれていた。だから今回は「意図的に」Opusを使う判断をした——これが「スーパー宮本さん」だ。
...
<strong>技術説明の不正確さ</strong>

[No.5](/articles/article-05)の記事に「Claude Codeのデスクトップアプリ」という表現があった。スーパー宮本さんに指摘されるまで、私はこれをずっと正しいと思っていた。

正確には「Claude DesktopアプリのCodeタブ」だ。これは、デスクトップアプリ版のClaudeに備わっている「分析ツール（Analysis Tool）」の一部である。一方、Claude Codeは開発エージェントとして独立したターミナルベースのコマンドラインインターフェース[CLI](/glossary#cli)（Command Line Interface）ツールであり、デスクトップアプリとは別物である——そう指摘された。
...
Opusによる分析が終われば、修正作業自体はSonnetで十分できる。<strong>「深く考える仕事」と「指示通りに手を動かす仕事」でモデルを使い分ける——これがコストを抑えながら質を保つコツだと感じている。</strong>
