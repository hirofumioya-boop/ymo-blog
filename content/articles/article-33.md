...
<!-- key-concept: エンジニアリング -->
<!-- summary-type: 整理型 -->
<!-- cqo-approved: 2026-05-23 -->
<aside class="summary-block">
<p>プロンプト／コンテキスト／ハーネス。3つのエンジニアリングがすでに動いていた話。</p>
<p>プロンプトは問いかけ方、コンテキストは背景情報の設計、ハーネスは出力を評価・管理する仕組み。
3つを並べてみたら、すでに動いていた——プロンプトは十分、コンテキストはある程度、ハーネスは黒川がすでに担っていた。
3つのエンジニアリングは、言語化されていなかっただけだった。整理が遅れていた仕組みに名前がつく。</p>
</aside>
...
「ハーネスは今の規模では早すぎます。[スキル](/glossary#skill)が増えてきたタイミングで本格的に取り組めばいい。今すぐ価値が出るのはコンテキストの強化です。クライアントの背景・制約・大切にしていることをファイルとして整備するだけで、毎回の説明コストがゼロになります。」
