...
<p>コードを書かない経営者が、気づいたらAPIを使い記憶を仕組み化していた話。</p>
<p>会話終了→ログをHaiku APIに送信→6人分のprofile.mdに追記。
コードを書けない経営者がAPIを動かしていた——気づいたらVibe Codingをやっていた。
7回呼び出しで3日50ドル。技術判断はエンジニアでなくてもできる。必要なのは何をしたいかの意思だ。</p>
</aside>
...
[No.5](/articles/article-05)で、AIの「記憶が消える」問題に触れた。
...
そしてもう一つ——この仕組みには、[Claude API](/glossary#api)を使っているという事実がある。コードを書いたことが一度もない、非エンジニアの私が。
...
ファイル名は [`memory_updater.py`](/glossary#memory-updater)。
...
「会話が終わったら自動的に記憶を書き出す」——[No.5](/articles/article-05)で書いた仕組みの正体は、これだ。
...
<strong>Claude自身がClaude APIを呼んでいる。</strong>

「自分自身の別バージョンに記録を書かせる」——そういう構造だ。

使っているのは[Claude Haiku](/glossary#claude-haiku)。高性能のSonnetやOpusではなく、軽量で低コストなモデルだ。記憶の書き出しは「要約」作業なので、高性能モデルを使う必要がない。コストを抑えながら、毎回の会話後に7つの記憶を自動更新できる。
...
[No.5](/articles/article-05)を書いたとき、「高橋さんが仕組みを作ってくれた」と一言で済ませていた。
...
<strong>コードが書けなくても、APIは使えた。設計の判断は、エンジニアでなくても下せた。</strong>必要なのは「何をしたいか」の意思と、「どうすればいいか」を一緒に考えてくれる相手だ。

私には高橋さんがいた。

最後に、余談を一つ。

最近、[**Vibe Coding**](/glossary#vibe-coding)という言葉が話題になっている。OpenAIの共同創業者、Andrej Karpathyが提唱した概念で、「コードの詳細を理解しなくても、AIに意図を伝えて実装してもらう」開発スタイルのことだ。
