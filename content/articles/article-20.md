...
そしてもう一つ——この仕組みには、[Claude API](/glossary)を使っているという事実がある。コードを書いたことが一度もない、非エンジニアの私が。
...
ファイル名は [`memory_updater.py`](/glossary)。
...
使っているのは[Claude Haiku](/glossary)。高性能のSonnetやOpusではなく、軽量で低コストなモデルだ。記憶の書き出しは「要約」作業なので、高性能モデルを使う必要がない。コストを抑えながら、毎回の会話後に7つの記憶を自動更新できる。
