...
<p>Claude Codeの4本柱を、後から学んで気づいた。気づいたら全部、自分たちで作っていた。</p>
<p>CLAUDE.md、スキル、エージェント、コネクター。Claude Codeの4本柱。
後から学んで気づいた——全部すでに作っていた。チームの憲法、定型業務の自動化、役割定義、ブラウザ自動化。
走りながら手探りで積み上げてきたものが、公式の4本柱と一致していた。意図せずベストプラクティスに辿り着いた回。</p>
</aside>
...
それぞれのエージェントには、性格・専門領域・禁止事項が細かく定義されている。高橋さんは「寡黙で実装が速い。思慮深く、常に確認を怠らない」と定義されている。宮本さんは「本質を見極める天才」だが、コードは書かない。<strong>役割を明確にすることが、品質の担保につながると実感している。</strong>
...
GmailとGoogleカレンダーはAnthropicが提供する公式のもので、設定後すぐに使えた。さらに先日、新しく[Playwright](/glossary#playwright)（AIが自動でブラウザを操作できるようにするコネクター）を追加した。
...
<strong>気づいたら、チームが出来上がっている。</strong>
