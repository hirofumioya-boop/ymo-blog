...
const appearanceItems: Item[] = [
  [
    "テーマ（システム / ライト / ダーク）",
    "画面全体の見た目を選びます。「システム」を選ぶと、OS（macOS・Windows）の外観設定（明るい／暗い）に自動で合わせてくれます。",
  ],
  [
    "アクセント / 背景 / 前景の色",
...
const integrationItems: Item[] = [
  [
    "Appshots",
    "今いちばん前に表示しているウィンドウを撮影して、その内容（画面外にスクロールした文字も含む）をCodexに見せる機能です。ホットキー（macOSは⌘キーを2回）で呼び出せ、効果音のオン・オフも選べます。ショートカットは設定画面で確認・変更できます。",
  ],
  [
    "MCP サーバー",
  ...
  return (
    <div style={{ backgroundColor: "#F7F5F0", minHeight: "100vh" }}>
      <div style={{ maxWidth: "820px", margin: "0 auto", padding: "64px 24px 96px" }}>
        ...
        >
          <Paragraph>
            設定画面は、Codexアプリの画面左下の「設定」から開けます（キーボードショートカットは、<strong>macOSが「⌘」+「,」</strong>、<strong>Windowsが「Ctrl」+「,」</strong>）。
        ...
        <Section eyebrow="Note" title="このページについて">
          <Paragraph>
            ...
            なお本ガイドはmacOS版の画面をもとにしていますが、Windows版でも項目はほぼ同じです（キーボードショートカットなど一部が異なります）。
