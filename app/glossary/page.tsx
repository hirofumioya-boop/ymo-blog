...
export const metadata = {
  ...
  alternates: {
    canonical: 'https://blog.y-m-o.jp/glossary',
  },
  openGraph: {
...
const terms = [
  ...
  },
  {
    ...
    reading: "ウェブフック",
    description:
      "ある出来事をきっかけに、別のサービスへ自動で通知を送る仕組み。たとえば「オートメーションが調査を終えたらSlackに結果を送る」「フォームに回答が来たら別のシステムへ知らせる」といった使い方をする。便利だが、通知先URLを知っている人は投稿できてしまう場合があるため、Webhook URLはAPIキーと同じように秘密情報として扱う。",
  },
  {
    id: "api",
    term: "API",
  ...
  },
  {
    ...
    reading: "エーピーアイキー",
    description:
      "APIを使うための認証用の文字列。サービス側に「この利用者からのアクセスです」と伝えるための鍵のようなもの。便利だが、他人に知られると勝手に使われたり、料金が発生したりする危険があるため、チャットに貼ったり、公開フォルダに保存したりしない。CodexやClaude CodeをAPI経由で使う場合にも、APIキーの扱いは最重要の安全ルールになる。",
  },
  {
    id: "asi",
    term: "ASI",
  ...
  },
  {
    ...
    reading: "コーデックス",
    description:
      "OpenAIが提供するAIエージェント系の開発支援ツール。作業フォルダを読み、ファイルを編集し、コマンドを実行し、変更内容を確認しながらタスクを進められる。ChatGPTに相談するだけでなく、実際のファイル操作まで任せられる点が特徴。このブログでは、作業環境を軽く整理しながら実行する相棒として使っている。",
  },
  {
    id: "claude",
    term: "Claude",
  ...
  },
  {
    id: "agents-md",
    term: "AGENTS.md",
    reading: "エージェンツ エムディー",
    description:
      "Codexに読ませる作業ルールを書くMarkdownファイル。Claude CodeにおけるCLAUDE.mdに近く、プロジェクトの目的、禁止事項、確認手順、よく使うコマンドなどを記録しておく。AIに毎回同じ説明をしなくてよくなる一方、内容が古いと判断を誤らせるため、短く保ち、必要に応じて更新することが大切。",
  },
  {
    id: "cao",
    term: "CAO",
  ...
  },
  {
    ...
    reading: "ディフ",
    description:
      "変更前と変更後の差分のこと。どのファイルのどの行が追加・削除・変更されたかを確認できる。AIにファイル編集を任せる場合、最終確認では完成画面だけでなくdiffを見ることが重要になる。Claude Codeの`/diff`やGitの差分確認は、AIとの共同作業における基本の安全確認。",
  },
  {
    id: "delegation",
    term: "Delegation",
  ...
  },
  {
    ...
    reading: "コンテキストセブン",
    description:
      "AIがライブラリやAPIの最新ドキュメントを確認するための外部サービス。AIは学習時点の古い知識で答えてしまうことがあるため、Next.js、Slack API、OpenAI APIなど仕様が変わりやすい技術を扱うときに、Context7で現在の公式ドキュメントを参照させる。このブログでは、CodexにContext7のMCP接続を追加し、古い手順や存在しないAPIに引っ張られないための補助輪として使う。",
  },
  {
    id: "cloud",
    term: "クラウド",
  ...
  },
  {
    ...
    reading: "ギット",
    description:
      "ファイルの変更履歴を記録・管理するための仕組み。いつ、誰が、どのファイルをどう変えたかを追跡でき、必要に応じて過去の状態へ戻せる。このブログのようなウェブサイトでは、AIが編集した内容を人間が差分で確認し、問題なければcommitして保存する流れで使う。GitHubはGitで管理した内容をオンラインに置くサービスで、Gitそのものとは別物。",
  },
  {
    id: "git-branch",
    term: "ブランチ",
    reading: "ブランチ（Branch）",
    description:
      "Gitで作業の流れを分ける仕組み。本番に反映されている流れとは別に、実験用・修正用の作業場所を作れる。AIに大きめの変更を任せるときは、ブランチを分けておくと、本線を壊さずに試せる。問題なければ後で本線に取り込む。",
  },
  {
    id: "git-commit",
    term: "commit",
    reading: "コミット",
    description:
      "Gitで、その時点の変更内容をひとまとまりとして保存すること。作業のセーブポイントのようなもの。後から「この時に何を変えたか」を確認でき、必要に応じて戻す手がかりにもなる。AIに編集してもらった後は、差分を確認してからcommitするのが基本。",
  },
  {
    id: "git-push",
    term: "push",
    reading: "プッシュ",
    description:
      "ローカル環境で保存したGitの変更を、GitHubなどのリモート側へ送ること。このブログでは、GitHubにpushするとVercelが公開サイトを更新する流れになっているため、pushは単なる保存ではなく公開につながる操作として扱う。AIに任せる場合も、人間の明示承認を挟むべき操作。",
  },
  {
    id: "github",
    term: "GitHub",
  ...
  },
  {
    ...
    reading: "ホームブルー",
    description:
      "macOSでアプリや開発ツールをインストール・更新するためによく使われるパッケージ管理ツール。ターミナルからコマンドで導入や更新を行う。Claude Codeのインストール方法の一つとしても登場する。便利だが、コマンドの意味がわからないまま実行せず、公式手順を確認して使うのが安全。",
  },
  {
    id: "hooks",
    term: "Hooks",
    reading: "フックス",
    description:
      "Claude Codeの操作前後など、特定のタイミングで自動的に処理を走らせる仕組み。たとえば、ファイル編集後に自動で整形する、作業前にチェックを入れる、設定変更時に通知する、といった使い方ができる。便利な自動化だが、何が実行されるかを把握しておくことが大切。",
  },
  {
    id: "iq",
    term: "IQ",
  ...
  },
  {
    ...
    reading: "オープンエーアイ",
    description:
      "ChatGPTやCodexを提供しているAI企業。生成AIを一般の人にも広く使える形にした代表的な企業のひとつ。このブログでは、Claude Codeと並ぶAI活用環境としてCodexを使う場面で登場する。AnthropicがClaudeを提供する会社であるのに対し、OpenAIはChatGPTやCodexの提供元。",
  },
  {
    id: "olp",
    term: "OLP",
  ...
  },
  {
    ...
    reading: "プルリクエスト",
    description:
      "GitHubなどで、ブランチ上の変更を本線に取り込んでよいか確認・依頼する仕組み。差分、説明、レビューコメントをまとめて扱える。チーム開発では、いきなり本番側に変更を入れるのではなく、プルリクエストで確認してから取り込むことが多い。AIが作った変更を人間がレビューする場としても使える。",
  },
  {
    id: "prompt",
    term: "プロンプト",
  ...
  },
  {
    ...
    term: "Slack Incoming Webhook",
    reading: "スラック インカミング ウェブフック",
    description:
      "外部ツールからSlackの特定チャンネルへメッセージを投稿するための仕組み。発行された専用URLに決まった形式のデータを送ると、Slackに通知が流れる。個人のCodex運用では、Codexのオートメーションが集めた要約や検証結果をSlackへ送る入口として試している。ただしURL自体が秘密情報なので、GitHubやチャットには貼らず、ローカルの環境変数にだけ置く。",
  },
  {
    id: "swot",
    term: "SWOT分析",
  ...
  },
  {
    id: "sandbox",
    term: "サンドボックス",
    reading: "サンドボックス（Sandbox）",
    description:
      "AIやプログラムが動ける範囲を区切る安全な作業領域のこと。ファイル編集を作業フォルダ内に限定したり、ネットワーク利用を制限したりすることで、誤操作や情報漏えいの影響を小さくする。CodexやClaude CodeのようなAIエージェントを使う時は、便利さよりもまずサンドボックスの境界を意識することが大切。",
  },
  {
    id: "5-whys",
    term: "5 Whys（なぜなぜ分析）",
  ...
  },
  {
    ...
    term: "役割同化",
    reading: "ヤクワリドウカ",
    description: "長期的に役割を演じ続けるうちに、AI の応答がその役割と一体化していく現象。近接する学術用語に、anthropomorphism（擬人化・人間側の認知傾向）、persistent identity（持続的同一性・AI が時間を跨いで一貫した人格を保つ性質）、role assimilation（役割同化の直訳）がある。本ブログでは、社長と AI 社員が継続的に経営課題を議論する中で、AI 側が「組織責任を背負う主体」として一貫して振る舞い続けた現象を指す。",
  },
  {
    id: "ui-ux",
    term: "UI / UX",
  ...
  },
  {
    id: "vs-code",
    term: "VS Code",
    reading: "ブイエスコード（Visual Studio Code）",
    description:
      "Microsoftが提供する無料のコードエディタ。ファイルを開いて編集したり、拡張機能を入れて開発作業をしやすくしたりできる。Claude CodeはVS Codeと連携でき、エディタ上で差分を見たり、作業内容を確認したりできる。コードを書かない人にとっても、AIが編集したファイルを確認する画面として役立つ。",
  },
  {
    id: "v0",
    term: "v0",
  ...
  {
    id: "vector-embedding",
    term: "ベクトル化（Embedding）",
    reading: "ベクトルカ（Embedding）",
    description:
      "テキストの「意味」を数値の列（ベクトル）に変換する技術のこと。たとえば「うれしい」「楽しい」「ハッピー」は言葉は違っても意味が近い。この近さを数値の距離として表現することで、キーワードがぴったり一致しなくても意味的に関連する情報を探し出せるようになる。表現の揺れや言い換えを吸収できるのが最大の特徴で、RAGの検索精度を高めるための基盤技術として広く使われている。",
  },
  {
    id: "loop-engineering",
    term: "ループエンジニアリング",
    reading: "ループエンジニアリング（Loop Engineering）",
    description:
      "AIが「繰り返し（ループ）」動く仕組みそのものを設計する技術のこと。指示を出して一度だけ答えを受け取る使い方とは異なり、AIが「計画→実行→確認→修正→再実行」というサイクルを自分でまわし続けることで、複雑なタスクを自律的にこなしていく。このループをどう設計するか——いつ止めるか、失敗したときにどう対処するか、人間の確認をどこで挟むか——を考えることがループエンジニアリングの核心になる。プロンプトエンジニアリング（何を指示するか）やコンテキストエンジニアリング（何を事前に渡すか）と合わせて、生成AIを「便利なツール」から「自律して動くシステム」へと進化させるための重要な設計領域になりつつある。このブログNo.45では、一言の指示をきっかけに、高橋さんの実装→黒川さんの差し戻し→修正→再レビューというサイクルが、社長不在のまま何度も自律的に回り続けた。その積み重ねでひとつの成果物が仕上がっていく——その構造が、ループエンジニアリングの考え方に近い。",
  },
  {
    id: "winget",
    term: "WinGet",
    reading: "ウィンゲット",
    description:
      "Windowsでアプリや開発ツールをコマンドからインストール・更新するためのパッケージ管理ツール。Claude CodeのWindows向けインストール方法の一つとして登場する。macOSでHomebrewを使うのに近い位置づけだが、使うOSによって手順が違うため、公式ドキュメントの対象OSを確認してから実行する。",
  },
  {
    id: "wsl",
    term: "WSL",
    reading: "ダブリューエスエル（Windows Subsystem for Linux）",
    description:
      "Windows上でLinux環境を動かすための仕組み。開発ツールの中にはLinux前提のものも多く、Windowsユーザーがそれらを使う入口になる。Claude Codeのインストール説明では、Windows上でLinuxに近い環境を使う選択肢として登場する。初心者は、通常のWindows手順とWSL手順を混同しないことが大切。",
  },
  {
    id: "generative-ai",
  ...
  },
  {
    ...
    term: "レスポンシブデザイン（Responsive Design）",
    reading: "レスポンシブデザイン",
    description:
  ...
  {
    id: "semantic-search",
    term: "セマンティック検索（Semantic Search）",
    reading: "セマンティックケンサク（Semantic Search）",
    description:
      "言葉が一致しているかではなく、「意味」が近いかで探す検索のこと。従来のキーワード検索は、入力した単語とまったく同じ文字列が含まれる文書しか見つけられない。たとえば「退職金」で調べると、同じ意味でも「会社を辞めるときに受け取るお金」と書かれた文書は取りこぼしてしまう。セマンティック検索は、文章の意味をベクトル化（数値化）してから距離の近さで探すため、言い換えや表現の揺れがあっても、意味的に関連する情報をすくい上げられる。専門用語の言い回しが人によって違っても対応しやすく、RAGで社内文書や過去事例を探す部分の中心的な技術になっている。",
  },
  {
    id: "ontology",
    term: "オントロジー（Ontology）",
    reading: "オントロジー（Ontology）",
    description:
      "ある分野の言葉と、言葉どうしの関係を、コンピュータが扱えるように整理した「概念の地図」のこと。単語を辞書のように並べるだけでなく、「消費税は税金の一種」「インボイスは請求書に関する制度で消費税とつながっている」といった“関係”まで定義するのが特徴だ。人間が経験のなかで暗黙に持っている「この言葉とこの言葉は近い／上下関係にある」という知識を明文化することで、AIや検索システムが言葉の意味を取り違えにくくなる。セマンティック検索やRAGの精度を、その分野ならではの言葉の関係を踏まえて底上げする土台として使われることがある。",
  },
];
