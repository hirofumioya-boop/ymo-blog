...
const terms = [
  ...
  },
  {
    ...
    reading: "コーデックス",
    description:
      "OpenAIが提供するAIエージェント系の開発支援ツール。作業フォルダを読み、ファイルを編集し、コマンドを実行し、変更内容を確認しながらタスクを進められる。ChatGPTに相談するだけでなく、実際のファイル操作まで任せられる点が特徴。このブログでは、YMOの作業環境を軽く整理しながら実行する相棒として使っている。",
  },
  {
    id: "claude",
    term: "Claude",
  ...
  },
  {
    ...
    description:
      "外部ツールからSlackの特定チャンネルへメッセージを投稿するための仕組み。発行された専用URLに決まった形式のデータを送ると、Slackに通知が流れる。このブログの運用では、Codexのオートメーションが集めた要約や検証結果をSlackへ送る入口として使う。ただしURL自体が秘密情報なので、GitHubやチャットには貼らず、ローカルの環境変数にだけ置く。",
  },
  {
    id: "swot",
    term: "SWOT分析",
