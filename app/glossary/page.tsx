...
const terms = [
  ...
  },
  {
    ...
    description:
      "AIが「繰り返し（ループ）」動く仕組みそのものを設計する技術のこと。指示を出して一度だけ答えを受け取る使い方とは異なり、AIが「計画→実行→確認→修正→再実行」というサイクルを自分でまわし続けることで、複雑なタスクを自律的にこなしていく。このループをどう設計するか——いつ止めるか、失敗したときにどう対処するか、人間の確認をどこで挟むか——を考えることがループエンジニアリングの核心になる。プロンプトエンジニアリング（何を指示するか）やコンテキストエンジニアリング（何を事前に渡すか）と合わせて、生成AIを「便利なツール」から「自律して動くシステム」へと進化させるための重要な設計領域になりつつある。No.45では、一言の指示を受けたAIチームが「実行→黒川さんの差し戻し→修正→再レビュー」を社長不在のまま何度も繰り返し、ひとつの成果物を仕上げた体験が描かれている。その「サイクルが自律的に回り続けた」構造こそが、ループエンジニアリングの考え方に近い。",
  },
  {
    id: "winget",
    term: "WinGet",
